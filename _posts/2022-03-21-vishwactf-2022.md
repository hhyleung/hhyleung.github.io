...
The next step was to read the file with `{{url_for.__globals__.__builtins__.open('flag.txt').read()}}`, yet it did not work. Seems like `flag.txt` was set to be bypassed as the other files like `app.py` can be read successfully with the injection. The file name was then replaced by hex characters to escape the checking. The payload `{{url_for.__globals__.__builtins__.open(%22\x66\x6c\x61\x67\x2e\x74\x78\x74%22).read()}}` bypassed the checking and returned the flag successfully.
